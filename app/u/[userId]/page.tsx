      ...
      <main className="wrap">
        ...
        <section className="section">
          <table>
            <thead>
              <tr>
                <th>コード</th><th>銘柄名</th><th>スコア</th><th>達成</th><th>未達★</th><th>タグ</th><th>現在値</th><th>損切り参考</th><th>損切り距離</th><th>利確20%</th><th>6か月値幅</th><th>詳細</th><th>株探</th>
              </tr>
            </thead>
            <tbody>
              {rows.length === 0 ? (
                <tr><td colSpan={13}>まだ分析結果がありません。CSV登録後、core の GitHub Actions を実行してください。</td></tr>
              ) : rows.map((r) => (
                <tr key={r.id}>
                  <td><b>{r.code}</b></td>
                  <td>{r.name || ''}</td>
